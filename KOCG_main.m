function [X_enumKOCG_cell, time] = KOCG_main(A, B, NG, alpha, beta)
...
fprintf('final value is %f',finalAns);
...
while (sum(glb_mask ~= 0))
    ...
    % find optimal X_loc 
    [X_loc, FX_Val_log] = FOCGCoreOuter(A(glb_mask, glb_mask), H(glb_mask, glb_mask), X_loc, U, NG, glb_mask);
    ...
    H_penalty = H(glb_mask, glb_mask)-beta*I(glb_mask,glb_mask);
    finalAns = finalAns + calcFXVal(A(glb_mask, glb_mask),H_penalty,X_loc,U);
    counter = counter + 1;
...
function [X, FX_Val_log] = FOCGCoreOuter(A, H, X, U, NG, glb_mask)
% initialize the global const variables
global conv_thres I beta;

% prepare Mi for i = 1
Xjs = X(:, 2:NG);
Mi = sum(H*Xjs, 2); % Mi when i = 1
H_penalty = H - beta*I(glb_mask,glb_mask);
%% Start iteration
FX_Val_log  = -1;
outer_iter = 0;
...
while(outer_iter <= outer_iter_thres)
    ...
    % log FX_Val
    cur_FX_Val = calcFXVal(A, H_penalty, X, U);
...
function [X_new] = TransformX(A,H,X,NG)
global I I_inv;
temp = A*X;
temp2 = H*X;
for i=1:NG
