function [X_enumKOCG_cell, time, finalAns] = KOCG_main2(A, B, NG, alpha, beta)
...
global shrink_tao conv_thres near_zero K finalAns;
finalAns = 0;
...
while (sum(glb_mask ~= 0))
    ...
    finalAns = finalAns + calcFXVal(A(glb_mask, glb_mask),H(glb_mask, glb_mask),X_loc,U);
    counter = counter + 1;
...
##H = H - beta*I;
FX_Val = trace(X'*A*X) + trace(X'*H*X*U) - beta*trace(X'*X*U);
